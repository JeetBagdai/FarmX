  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <input
        ...
        className="w-full px-4 py-2 border border-gray-600 bg-gray-700 text-gray-200 rounded-md shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition placeholder-gray-400"
        required={required}
        autoComplete="off"
      />
      {isOpen && (
        <ul className="absolute z-10 w-full mt-1 bg-gray-700 border border-gray-600 rounded-md shadow-lg max-h-60 overflow-y-auto">
          {filteredOptions.length > 0 ? (
            ...
            <li className="px-4 py-2 text-gray-400">{t('No regions found. You can still enter a custom region.')}</li>
          )}
        </ul>
      )}
    </div>
